...
function AdminUserManagement({ embedded = false }) {
  ...
  return (
    <div className={embedded ? '' : 'max-w-6xl mx-auto bg-white p-6 rounded-lg shadow border border-gray-200'}>
      {!embedded && (
        <h2 className="text-2xl font-bold text-primary mb-4">{t('settings.userManagement.title')}</h2>
      )}
                ...
                {filteredUsers.length === 0 ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 border-b text-sm">{user.id}</td>
                      <td className="px-4 py-3 border-b text-sm font-medium">{user.username}</td>
      ...
      {activeTab === 'bannedIPs' && (
        <>
          <p className="text-sm text-gray-600 mb-4">{t('settings.bannedIPs.description')}</p>
                ...
                {bannedIPs.length === 0 ? (
                  ...
                  bannedIPs.map((ip) => (
                    <tr key={ip.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 border-b text-sm font-mono">{ip.ip}</td>
                      <td className="px-4 py-3 border-b text-sm">{ip.reason || '-'}</td>
                      <td className="px-4 py-3 border-b text-sm text-gray-500">
      ...
      {showBanDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">{t('settings.userManagement.banDialog.title')}</h3>
            <div className="mb-4">
