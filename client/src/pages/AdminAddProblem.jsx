      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6 bg-gray-50 p-4 rounded border border-gray-200">
            <div>
                ...
                <input required type="number" name="timeLimit" value={form.timeLimit} onChange={handleChange} className="w-full border border-gray-300 p-2 rounded" />
            </div>
            <div>
                <label className="block text-gray-700 font-bold mb-2">{t('problem.add.memoryLimit')}</label>
                <input required type="number" name="memoryLimit" value={form.memoryLimit} onChange={handleChange} className="w-full border border-gray-300 p-2 rounded" />
            </div>
        </div>
        ...
        <div className="bg-yellow-50 p-4 rounded border border-yellow-200">
            ...
            <div className="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="cppTimeLimit" value={form.cppTimeLimit} onChange={handleChange} placeholder="e.g. 500" className="w-full border border-gray-300 p-2 rounded" />
                </div>
                <div>
                    <label className="block text-gray-700 text-sm mb-1">{t('problem.add.pythonTimeLimit')}</label>
                    <input type="number" name="pythonTimeLimit" value={form.pythonTimeLimit} onChange={handleChange} placeholder="e.g. 2000" className="w-full border border-gray-300 p-2 rounded" />
                </div>
            </div>
            <div className="mt-4">
                 <label className="block text-gray-700 text-sm mb-1">{t('problem.add.cppCompileOptions')}</label>
                 <input type="text" name="defaultCompileOptions" value={form.defaultCompileOptions} onChange={handleChange} className="w-full border border-gray-300 p-2 rounded font-mono text-sm" />
            </div>
        </div>
        ...
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={addTestCase} className="text-primary hover:text-blue-700 font-bold">{t('problem.add.addCase')}</button>
            </div>
            
            {testCases.map((tc, index) => (
                <div key={index} className="mb-4 p-4 border border-gray-200 rounded relative bg-gray-50">
                    <button type="button" onClick={() => removeTestCase(index)} className="absolute top-2 right-2 text-red-500 hover:text-red-700">{t('problem.add.remove')}</button>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase">{t('problem.add.input')}</label>
                            <textarea value={tc.input} onChange={(e) => handleTestCaseChange(index, 'input', e.target.value)} rows="2" className="w-full border border-gray-300 p-2 rounded font-mono text-sm"></textarea>
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase">{t('problem.add.expectedOutput')}</label>
                            <textarea value={tc.expectedOutput} onChange={(e) => handleTestCaseChange(index, 'expectedOutput', e.target.value)} rows="2" className="w-full border border-gray-300 p-2 rounded font-mono text-sm"></textarea>
                        </div>
                    </div>
                </div>
            ))}
