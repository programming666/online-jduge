...
function UserCode() {
  ...
  return (
    <PageTransition>
      <div>
        <h2 className="text-2xl font-bold text-primary mb-4">{t('user.code.title')}</h2>
            ...
            <tbody>
              {items.map(s => (
                <tr key={s.id} className="border-t hover:bg-gray-50">
                  <td className="px-4 py-2 text-sm">{new Date(s.createdAt).toLocaleString()}</td>
                  <td className="px-4 py-2 text-sm">{s.problemId}</td>
                  <td className="px-4 py-2 text-sm">{s.language}</td>
                  <td className="px-4 py-2"><span className={getStatusBadge(s.status)}>{s.status}</span></td>
                </tr>
              ))}
              {items.length === 0 && !loading && (
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-gray-500">{t('user.code.empty')}</td>
                </tr>
              )}
            </tbody>
          </table>
